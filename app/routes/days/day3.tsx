...
const getBitCounts = (report: string[]) => {
    const bitCounts = [...Array(report[0].length)].map((e) => Array(2).fill(0));
...
export default function Day3() {
    ...
    const createContent = () => {
        ...
        const part1 = () => {
            const gammaRate = Number(
                `0b${bitCounts.reduce(
                    (p, c) => p + (Number(c[0]) > Number(c[1]) ? "0" : "1"),
                    ""
                )}`
            );
            ...
            const epsilonRate = Number(
                `0b${bitCounts.reduce(
                    (p, c) => p + (Number(c[0]) > Number(c[1]) ? "1" : "0"),
                    ""
                )}`
            );
        ...
        const part2 = () => {
            ...
            for (let i = 0; i < report[0].length; i++) {
                ...
                filteredNumbers = filteredNumbers.filter(
                    (x) =>
                        x[i] === (Number(b[i][0]) > Number(b[i][1]) ? "0" : "1")
                );
            ...
            for (let i = 0; i < report[0].length; i++) {
                ...
                filteredNumbers = filteredNumbers.filter(
                    (x) =>
                        x[i] === (Number(b[i][0]) > Number(b[i][1]) ? "1" : "0")
                );
    ...
    return <Day content={createContent()} fileData={fileData} day={day} />;
}
